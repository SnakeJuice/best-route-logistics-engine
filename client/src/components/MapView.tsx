...
// Corregir la ruta de los íconos por defecto de Leaflet
...
// Coordenadas base: Santiago Centro
const SANTIAGO_CENTER: [number, number] = [-33.4442, -70.6528];

export const MapView: React.FC<MapViewProps> = ({ orders, routes }) => {
  return (
    <div style={{ height: '550px', width: '100%', borderRadius: '12px', overflow: 'hidden', border: '1px solid #ccc' }}>
      <MapContainer center={SANTIAGO_CENTER} zoom={12} style={{ height: '100%', width: '100%' }}>
        <TileLayer
        ...
        {/* 2. Trazado de Rutas Optimizadas y sus Paradas */}
        {routes.map((route) => {
          ...
          return (
            <React.Fragment key={route.id}>
              {/* Línea Azul con el trazado exacto de OSRM */}
              <Polyline positions={positions} color="#2563eb" weight={5} opacity={0.8} />

              {/* Paradas asignadas a esta ruta */}
              {route.stops.map((stop) => (
                <Marker key={stop.id} position={[stop.order.latitude, stop.order.longitude]}>
                  <Popup>
